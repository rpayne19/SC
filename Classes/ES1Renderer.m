...
- (void) initOpenGL {
   ...
   // glScalef(screenBounds.size.width/480, screenBounds.size.height/320, 1); //messing with the screen
	//glOrthof(0, screenBounds.size.width, 0, screenBounds.size.height, -1, 1); //was left justified
    glOrthof(0, screenBounds.size.width, 0, screenBounds.size.height, -1, 1);

    // Set the viewport
    //glViewport(0, 0, screenBounds.size.width, screenBounds.size.height);  //was left justified
    glViewport(0, 0, screenBounds.size.width, screenBounds.size.height);
#ifdef SCB

    NSLog(@"INFO - ES1Renderer: Setting glOrthof to width=%f and height=%f", screenBounds.size.width,screenBounds.size.height);
#endif
