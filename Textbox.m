...
- (id)init{
    self = [super init];
    image = [[[Image alloc] initWithImageNamed: @"graytextbox.png" filter: GL_NEAREST] retain];
...
- (void)updateWithDelta:(GLfloat)aDelta scene:(AbstractScene*)aScene {
    ...
    switch (state) {
        case kEntityState_Alive:
          ...
          //  if(timer > .002f && [text length] > textIndex){ //was .02f
                if(timer > .02f && (([text length] > textIndex && currentLine == 1) ||
        ...
        //              [sharedSoundManager playSoundWithKey:@"voice1" location:CGPointMake(240, 160)];
                        if(textIndex % 2 == 0)
                            [sharedSoundManager playSoundWithKey:@"voice2" gain:.08f pitch:1.2f location:CGPointMake(240, 160) shouldLoop:NO];
                    }
            ...
            if((lifeTimer >= 3.8 && numberOfLines ==2) || (lifeTimer >= 1.9 && numberOfLines == 1) || (lifeTimer >= 5.7)){ //was 3.5
...
- (void)render {
    if(state == kEntityState_Alive) {
		[super render];
        [image renderCenteredAtPoint:CGPointMake(240,261)];
        if(textboxSize >= 1) {
            [super render];
            
            if(currentLine > 1) {
                //output text1 if on another text line
                [smallFont renderStringJustifiedInFrame:CGRectMake(12, 260, 76, 35) justification:BitmapFontJustification_MiddleLeft text:text];
                if(currentLine > 2) {
                    //output text2 if on text3
                    [smallFont renderStringJustifiedInFrame:CGRectMake(12,242,76,35) justification:BitmapFontJustification_MiddleLeft text: text2];
                    //output the current location of text3
                    [smallFont renderStringJustifiedInFrame:CGRectMake(12, 224, 76, 35) justification:BitmapFontJustification_MiddleLeft text:[text3 substringToIndex:textIndex]];

                }else   //output current location on text2
                    [smallFont renderStringJustifiedInFrame:CGRectMake(12, 242, 76, 35) justification:BitmapFontJustification_MiddleLeft text:[text2 substringToIndex:textIndex]];
 
            
        }
        else    //output the current location on text1
            [smallFont renderStringJustifiedInFrame:CGRectMake(12, 260, 76, 35) justification:BitmapFontJustification_MiddleLeft text:[NSString stringWithFormat:[text substringToIndex:textIndex]]];
        }
    }
}
