...
#define RIGHT DEGREES_TO_RADIANS(0)
#define UP DEGREES_TO_RADIANS(90)
#define LEFT DEGREES_TO_RADIANS(180)
#define DOWN DEGREES_TO_RADIANS(270)
...
enum  type {
	kObjectType_Key,        //0
	kObjectType_Energy,     //1
	kObjectType_General,    //2
    kObjectType_Textbox,    //3
    kObjectType_Background, //4
    kObjectType_TextEvent,  //5     subtype = the number of lines of text
    kObjectType_Checkpoint  //6
};
...
enum subtype {
    ...
    kObjectSubType_SockTrap = 25,
    kObjectSubType_RFID = 26
};
...
static inline CGPoint pixelToTileMapPosition(CGPoint tmp){
    return CGPointMake((float)(tmp.x/ kTile_Width), (float)(tmp.y / kTile_Height));
}
