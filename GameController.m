...
- (void)transitionToSceneWithKey:(NSString*)aKey {
    ...
    if([aKey compare:@"menu"]){
       [[gameScenes objectForKey:@"game"]dealloc];
        //NOT USED TO LOAD THE GAME
        AbstractScene *scene = [[GameScene alloc] initWithMap: @"gameMap" music: @"sneak1.mp3" location:CGPointMake(11, 11)];

        [gameScenes setValue:scene forKey:@"game1"];
        currentScene = [gameScenes objectForKey:@"game1"];
	...
	// Run the transitionIn method inside the new scene
        ...
        NSLog(@"GameController: Else clause: loading scene with key: %@", aKey);
#endif
    ...
    NSLog(@"Transitioning into game scene ");
#endif
...
- (void)initGameController {
	...
	NSLog(@"initialized menu scene");
#endif

	// Game scene
  //  nextScene = [GameScene alloc];
	scene = [[GameScene alloc] initWithMap: @"gameMap" music: @"sneak1.mp3" location:CGPointMake(11, 11)];
	...
	shouldResumeGame = NO;
  //  if(currentScene == [gameScenes objectForKey:@"game"])
    //    [currentScene configureGameControllers];
