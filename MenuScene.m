...
- (id)init {
	...
	if(self = [super init]) {
		...
		menu = [[[Image alloc] initWithImageNamed: @"sctitlescreenfinal.png" filter:GL_NEAREST] retain];
		...
		fadeImage.scale = Scale2fMake(1200, 1200);
		// Define cloud speed and position
		zoomDelta = 3.8;
		
        menu.scale = Scale2fMake(zoomDelta, zoomDelta);
		...
		startButtonBounds = CGRectMake(screenBounds.origin.x, screenBounds.origin.y, screenBounds.size.height, screenBounds.size.width);
...
#ifdef SCB
	NSLog(@"returning a copy of the menu scene after initialization");
#endif
    return self;
	...
	switch (state) {
		case kSceneState_Running:
			if(zoomDelta >= 1.5){
                ...
                menu.scale = Scale2fMake(1.0f, 1.0f);
			}
		...
		case kSceneState_TransitionOut:
			...
			if(alpha > 1.0f) {
				...
				[fadeImage renderAtPoint:screenBounds.origin];
...
- (void)transitionIn {
	...
	[sharedSoundManager loadMusicWithKey:@"menu" musicFile:@"Spotted.mp3"];
	//[sharedSoundManager removePlaylistNamed:@"menu"];
	[sharedSoundManager addToPlaylistNamed:@"menu" track:@"menu"];
